...
| `agent-provider-lifecycle.steps.ts`  | Rejects invalid drafts and drives two Providers through rollback, reload, dispatch, and cleanup |
| `conversation-context-copy.steps.ts` | Drives user/assistant prefixes, streaming completion, cancellation, reload, and isolation       |
...
| `session-goal.steps.ts`              | Drives isolated goal update, Pause, Resume, Clear, reload, Archive, and cleanup                 |
...
| `sidebar-search.steps.ts`            | Drives three Sessions through query variants, rename, reload, Archive, and cleanup              |
| `text-attachment.steps.ts`           | Drives picker cancel, attachment and plain turns, reload, isolation, and cleanup                |
