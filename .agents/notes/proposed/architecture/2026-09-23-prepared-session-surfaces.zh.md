...
显示中位数约 41 ms、文字插入确认约 90 ms。产品接入仍是提案，缓存未命中与独立副本仍是明确限制。
...
不满足这个行为。Provider 的中性备用窗口及后台预取规则目前禁止推测性获取会话 UI。
实现前需要为已准备视图明确草案契约并更新所属规则，不能偷偷复用只负责原始快照的后台预取机制。

## 建议顺序

1. **绑定目标的已准备视图。** 先沿用现有原生窗口，只保留一个隐藏的目标窗口。根据会话菜单
   打开、修饰键悬停／聚焦等高置信意图，以及容量受限的最近目标缓存，提前完成真实历史加载、
...
产品预渲染副作用、真实命中率、内存预算及签名 arm64／x64 分发均未验证。

## 结论

优先实验目标已绑定、完全可交互的预热视图。共享所有权作为独立架构迁移，GPU 预览作为可选
视觉机制。原型不改变产品运行行为或 Spec 保证。文档检查仍受既有缺失子模块断链影响，
