...
Layer 3 validation: full `TMPDIR=/private/tmp NODE_ENV=test pnpm check` passes, including 479 component files / 3,670 tests. Queue preparation uses the existing WorkspaceWriter and retains queue format. Native queue-steer keeps the queued journal identity but first promotes its delivered queue operation back to saved history work; the history turn is durably prepared and committed before the queue row can be removed or guide delivery begins. A committed record with an unknown result may be explicitly discarded only after disclosure that it may already have sent; destructive logout/cache-clear offers the same disclosed exit. `pnpm format` and docs check completed; docs report zero errors. No packaged-device acceptance is claimed.
