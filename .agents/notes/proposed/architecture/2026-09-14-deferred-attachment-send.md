...
Layer 3 validation: full `TMPDIR=/private/tmp NODE_ENV=test pnpm check` passes, including 479 component files / 3,670 tests. Queue preparation uses the existing WorkspaceWriter and retains queue format. Native queue-steer reuses the existing queued journal entry instead of admitting a conflicting `pending_apply` duplicate; guide/delivery changes that existing record and retains its recovery identity. `pnpm format` and docs check completed; docs report zero errors. No packaged-device acceptance is claimed.
