...
Pending presentation belongs in the conversation stream, not a separate recovery card or composer queue. After Send, a new conversation opens immediately and a continuation stays on its current conversation. Until the history entry can be committed, the local pending record renders as the ordinary right-aligned user-message row: attachment previews/cards carry their individual preparation or upload progress, the message row says it is waiting to send, and failure keeps that same row with retry/cancel actions. This preserves the user’s message context and avoids representing the same pending input in a second, disconnected surface.
