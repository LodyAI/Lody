...
### 2026-09-26 — 消除重复公钥验证与快照启动工作

- 保留另一个进行中的移除 `canManage` 改动。对照基线是 HEAD `7a52a45d`
  上当前工作区的冻结副本，不混用线格式已不兼容的 `d7d3b7c6`。
  基线 `src/**/*.ts` 指纹：
  `91fac57be47ab0cb8790695fdb8316a409d2cf7eb0709025f2be1bf97c5eb030`。
- 解码第一遍即收集不可变的公钥有效性证据。私有解码回调不对外开放。
  SignatureVerifier 仅按精确字节复用这些证据，仍检查每个签名的 R 子群与
  签名方程。权限重放不变；公钥有效不代表有权操作。批量验签也持有延迟执行
  输入的副本。
- 快照历史前缀扫描跨页保留公钥证据。Promise 快照工厂复用原生工厂的引擎，
  落盘后保留已验证视图。已有 journal 仍核对长度、head、认证状态摘要与
  快照原文，保留 pending 和游标。打开后、首次刷新前被替换成同 head 的另一
  快照也会拒绝。
- 新增真实密码学回归：有公钥证据时仍拒绝修改的消息/签名/公钥；延迟批处理
  可重复运行；嵌套公钥收集不修改输入；拒绝晚到的快照替换。本次性能修复不改
  协议或权限，不新增全局缓存或免验证构造入口。
- `bench/effect-compare.ts` 测原生完整重放、完整意图提交、快照加首次同步、
  SQLite 恢复加刷新，额外测 Promise 快照入口。双方共用 1000 条真实签名记录，
  预热 3 次、交替采样 10 次，每轮/客户端新建验证器。基线目录：
  `/private/tmp/e2ee-point-perf-xDBtJ9/before`。这是进程已预热、客户端冷启动，
  不是浏览器、手机或进程启动的证据。首轮基准因空 SQLite 尚未延迟创建而停止；
  已在计时前明确初始化空存储，仅修正测试工具，不改运行时代码。
- 配对中位数（Node 22.23.1，毫秒，修复前 → 后）：重放 1902.83 →
  1461.36（-23.2%）；完整提交 83.88 → 79.19（-5.6%）；原生快照
  779.33 → 421.79（-45.9%）；恢复 1925.37 → 1508.92（-21.6%）；
  Promise 快照 970.25 → 444.68（-54.2%）。原始样本：
  ...
  指纹计算：按源码相对路径排序所有 `.ts` 文件，依次连接路径、NUL、内容、NUL
  做 SHA-256。这只确认当前格式修复前后的性能改善，不代替原始迁移基线对比
  或 P0–P4 的全部验收。
- 验证：Core `vitest run --maxWorkers=2`：496 通过、1 跳过；10k 持久化与
  重启用例在 125.08 秒通过，没有改变原来的 180 秒超时。Lab 全量 142 通过。
  Core/Lab 类型检查、Electron `typecheck:node`、完整 Effect 分层检查、局部
  类型感知 lint（0 错误、30 警告）、修改源码格式检查与 `docs check` 通过。
  Lean 可执行模型对照仍由原有的模型可用性/策略同步检查跳过，本次修复不是
  Lean 证明。未重新运行根目录 `pnpm check`。
- 最后仅为 lint 显式返回 `undefined`，不改变基准行为；最终源码指纹为
  `1eefee403fabf0b9aeb117de27376220cf3c5c6a08b6ad7b8b6b825e2bfbf7a7`。
  未 commit/push，保留其他进行中的改动，产品 E2EE 仍关闭。
