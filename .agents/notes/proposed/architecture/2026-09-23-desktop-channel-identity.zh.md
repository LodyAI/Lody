...
下载页提供独立 Nightly 区域及中英文真实数据、手动切换说明。公开的构建时下载地址为
可选配置；清单不可用时不显示 Nightly 链接。解析器要求六个同版本安装包完整存在，
只在配置的下载根地址下生成链接，不接受清单中的任意 URL 或正式版别名。关于页面直达
Nightly 区域。浏览器 smoke 覆盖桌面/手机尺寸与完整清单、404 响应；部署地址、CORS
和真实安装包下载仍待验证。
清单还必须记录已经验证的最低正式版，下载链接前展示该要求。发行自动化检查操作者确认的
正式版本已经发布；版本数字比较本身不能证明兼容。兼容正式版上线与真实跨平台验收仍是前置条件。
...
保留轮廓与布局。格式转换不能证明安装后系统图标或小尺寸托盘的可辨识度，
这些仍需安装包视觉验收。
