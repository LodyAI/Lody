...
启动现在在原有 90 秒总期限内重试 ENETUNREACH/EHOSTUNREACH。有界遍历涵盖
连接聚合错误及 cause，保留安全错误码和校验后的 IP/地址族信息。包含永久错误
时不重试。共享的代理感知 HTTP 传输为直连和代理连接显式启用 Node 地址族选择，
其其他调用者也受益；不绕过代理，不固定 Cloudflare 地址。显式选择 Node 默认
传输的模式仍由 Node 控制。健康检查保持五秒上限且不采用启动重试，不增加隧道
重建，也不修改机器 DNS。

确定性用例覆盖 DNS 预算结束后的网络恢复、聚合错误、证书失败、取消和总期限。
真实本机 HTTP 与 CONNECT 边界验证运行时默认地址族选择关闭时，首个 IPv6 地址
不可用后仍能通过 IPv4 完成请求，无需依赖外部网络即可证明回退及代理路由行为。

验证：114 项预览与 HTTP 传输测试通过。独立显式启用的真实 Quick Tunnel 测试
也在 Linux 通过，涵盖公网 HTTP 转发、匿名访问拒绝、二进制 WebSocket 回显、
活跃健康检查和撤销；关闭隧道后合成本地服务器仍可访问。这证明了一次真实运行，
不代表对外部服务可用性的保证。
全仓类型、lint、格式、文档和边界检查通过；完整 `pnpm check` 仍因已有的
virtua `act is not a function` 测试错误中止。
