...
[English](2026-09-10-windowed-reader-integration.md)
...
已接受历史的同步确认会替换显示层包装对象，但底层会话可能完全未变。
按包装对象身份重置首屏就绪状态，会在新范围加载确认期间再次隐藏已显示的聊天，
也会把阅读窗口重置到尾部。就绪状态和可见范围现在归属于 `factSource ?? view`，
范围租约仍针对当前投影获取。底层源变化时，即使会话 ID 相同，也重新等待加载。
仅按会话 ID 缓存就绪状态，会导致替换后的文档在加载完成前提前显示。

[Hook 回归测试](../../../../packages/components/tests/conversation-view-hooks.test.tsx)
通过显式控制新范围的 promise，检查多次刷新、移除投影时的可见性及非尾部范围保留。
可见性断言在旧实现上失败。源替换测试还覆盖前一个源尚未就绪及已经显示两种状态。
[RefreshAcceptedHistory](../../../../packages/components/src/stories/ConversationViewStream.stories.tsx)
用 3,000 条合成 Turn 运行真实列表。生产构建的浏览器采样中，三次刷新原先每次
都会切换可见性；修复后均不再切换，行数、滚动偏移和高度保持不变。37 项相关
Hook/滚动测试、组件类型检查和生产 Storybook 构建通过。这修复的是已证实的
重新隐藏路径，不代表所有“进入后闪烁”的报告都已归因于此。
