...
`renderer-process-gone.test.mjs` 覆盖了 clean-exit 排除和崩溃负载，包括稳定的遥测字段与恢复详情。`error-boundary-manual-recovery.test.tsx` 证明 reset-key 变化会保留崩溃页，且只有用户重试才会渲染健康子树。尚未在打包桌面构建中强制制造真实的渲染进程崩溃。
