...
[Benchmark README](../../../../packages/components/benchmarks/README.md)说明命令、数据及方法；
[原始数据](../../../../packages/components/benchmarks/results/mention-file-search-2026-09-20.json)
...
生产 Worker 构建均通过。完整组件类型检查受本 checkout 缺失的工作区依赖（含 Electron）
和无关诊断阻挡。最终文档检查仍有 11 个指向未初始化 Codex/Grok 子模块的既有断链，
均不属于此次文档修改。未执行完整 Electron 构建或界面性能录制。
