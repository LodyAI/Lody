...
首次复审用明确的 `it.fails` 记录回归；当时套件绿色代表成功复现已知失败，
不是已修复。应在精确变化 key 转发或完整性修复路径真正补齐接收端后移除标记。
换成 inclusiveVersion 不能解决。该轮仅改测试和证据，后续生产修复记录如下。

继续审查生命周期，新增 4 条通过的测试：卸载/重开保留 checkpoint 并拒绝旧句柄；
purge 使另一个活跃副本的旧代际失效；旧 checkpoint 最后保存仍保留两端数据；
Meta 和 named Flock 的同版本向量 tombstone 在重开后仍能阻止旧记录复活。
卸载和 purge 合并在一条生命周期测试中。这些是确定性的 IndexedDB/Flock 测试，
不是物理崩溃测试。两个 renderer 套件现有 31 条测试通过，components 类型检查
通过。这些路径未确认额外 P0/P1；当时本地同步修复传播缺口仍未解决。

## 本地链路修复实现

两端现在按连接记录每个 key 的实际记录，不再用 peer 最大时钟过滤 Flock 导出。
首次连接/重连清空该缓存，核对全部记录；renderer 的旧 haveVersion 字段发送空
向量，让旧 v7 服务端也提供完整数据。在线 Flock 事件合并变化的 key，通过
getEntry 点读，保留时钟、值、metadata 和删除记录。缺少 key 事件/getEntry 的
结构适配器退回全量导出再按记录过滤。收到的记录用于抑制回声，renderer 不再
忽略云端导入事件。发送失败会在重试时重新核对。原有分块、调度器和按 peer
背压保留。

原预期失败测试已改为正常回归。真实 Flock 双端测试覆盖首次连接前与离线期间
同版本向量的缺口、双向在线云导入、在线更新不全表导出，以及最终不再发送回声。
异步导出交错测试改用显式屏障，不再靠 sleep。4 个本地链路套件 53 条测试通过。
JSON wire 格式仍为 v7；旧端升级前不具备新的在线补齐保证。代价是重连时完整
核对和每条连接按已见记录量占用内存。该修复不解决历史远端 snapshot 缺失，也
不改变 SQLite 暂用内存 checkpoint 的策略。
