...
**主数字旁边那块空要的是内容，不是装饰。** 为了用品牌标记填它，我试了四版：把
`lody.svg` 描边成轮廓（同一张卡上出现两只不同的水母）、把 `lody-icon.png` 低不透明度
淡化（读作角落里的第二个 logo）、贯穿全高的剪影（热力图和图例这两块最密的带背后压上
投影）、以及用上边缘裁切的剪影（伞盖被切平，成了压在区间胶囊底下的一块污渍）。每一版
都修好了上一版的症状，却没有一版触及病根：淡化的 logo 只是拿装饰冒充内容。

该放那儿的是这个区间自己的形状——主数字所计的那段周期，每个 bucket 一根柱子。卡片本来
已经有 53 周的年度热力图和总量，却始终没说**这三十天**长什么样，而这恰恰是主数字自己
说不出来的那件事。任何区间都画得出来（每个区间都有 `timeline.buckets`），长周期按求和
折叠到固定槽数，因此总量始终诚实。卡片正文里不再有品牌标记——表头和页脚本来就带着它。
