...
品牌标记改了三版才不再是问题。第一版把 `lody.svg` 描边成轮廓，结果同一张卡上出现了
两只不同的水母——那个矢量并不是产品图标，而产品图标没有矢量版本，CSS 也无法从位图切出
真正的轮廓。第二版改用 `lody-icon.png` 靠不透明度淡化，身份对了，但读起来仍是「角落里
又放了一个 logo」。最终的做法是把它当作投影而非标记：`brightness-0` 把图形压成纯剪影
（透明度保留、颜色不保留），深色卡上再反相让投影变亮，并放大。位置和处理一样反复：
让它贯穿整张卡的高度，热力图和图例这两块最密的带背后都压上了投影；改用卡片上边缘去裁
以收住范围，又把伞盖齐齐切平，留下一块压在区间胶囊底下的污渍。成立的做法是只从**一条**
边——右边——出血，上不越过品牌行、下不越过标题格的分隔线，于是图形保持完整、裁切看起来
是选择而不是意外。横版则完全不放：它没有空白可填，唯一放得下投影的位置正好压在热力图
被点亮的那一端，而那恰恰是卡片要强调的窗口。标记定位在卡片根节点上、所有信息带都压在
它之上，因此永远不会挤走任何一个数字。
