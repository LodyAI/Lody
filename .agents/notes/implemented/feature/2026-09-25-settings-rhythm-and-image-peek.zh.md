...
实际渲染的方案中选定：导航、中性灰画布、画布上的白卡片三层，用底色而非线条区分。第四轮评审把
桌面设置改成平铺：白色页面上直接放标题和行，每个分组上方一条细线，没有卡片。紧贴
...
Owner 觉得页面“全是线条”，要求导航向黑色混合改为 3.5%，内容区改成 Linear 设置页的样子。
导航改为 3.5% 后与原来的画布同色，所以内容区改用面板自身的底色（`surface.canvas` 为
`elevatedBackground`），两种配色下导航都比它低一级。

先按 Linear 的做法实现：每个分组是一块托盘（2% 墨色薄层、一条 1px 描边、无阴影），
行分隔线在行文字处收住，标题、分组标题和托盘左边缘对齐。Owner 认为不好看，问不要带框的
卡片会怎样。于是在两种配色下渲染了两个平铺方案：A，每个分组上方一条线、行之间无线；
B，在 A 的基础上加行间的内缩分隔线。Owner 选了 A。

现在分组就是页面上的标题和行。每个分组上方的细线是唯一的线条，没有标题的分组也能看出
在哪里结束。行向两侧伸出 16px，使行文字与标题对齐，并给悬停底色留出空间，悬停底色带圆角。
危险分组没有边框可标，改为标题显示为红色。行内边距、分组间距和字阶不变。

平铺只作用于特定范围，而非全局。`surface.card` 和 `surface.lineRuled` 也用于移动端设置页
和设置对话框里的行，它们保留卡片。`settings/material.stylex.ts` 把分组的画法定义为一组变量
（底色、阴影、圆角、外伸、行分隔线、标题缩进、分组上方细线、危险描边、空状态底色），
默认值是卡片；`settingsFlat` 是桌面设置窗口和项目窗口套用的主题。机器列表和技能列表里
手写的行分隔线改用 `surface.lineRuled`，同样跟随主题。
...
- 第四轮：设置浮层（偏好设置、外观、MCP、项目、键盘快捷键、关于），两种配色、中文，
  包括托盘版和两个平铺方案；材质对比 story 在 `settingsFlat` 之外，仍绘制卡片。
