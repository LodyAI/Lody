...
owner 仍觉得账号页“乱，有种说不出来的负担”。对着截图数：一页里有三种版式（平铺行、每行六个控件的
记录卡片、空卡片上方的标题操作），七种可点击的样式，右边缘至少三条。第一个标题“个人资料”旁边挂着
邮箱，看上去只是又一行。全页墨色最重的是品牌 Logo 和 Agent 图标堆叠。退出登录夹在个人资料和机器
列表之间。

- **一种版式。** 账号页每组都是带标题的卡片：个人资料（邮箱、用户名、头像）、已绑定账号、我的机器、
  CLI Token、登录（密码，然后是全页最后的退出登录）。个人资料是身份信息而不是偏好，所以在
  `settingsFlat` 里也用卡片。邮箱单独成行。
- **机器行共享列。** 所有行组成一个网格，每行是它的 `subgrid`：名称占剩余宽度，后面每列取该列最宽
  单元格的宽度。原来每行的控件从右边缘排起，少一个 Agent 图标的机器整组会偏移约 30px。现在每格都是
  纯文字（“6 个 Agent”“10 个目录 ▾”，然后是设置按钮）。Agent 名称移进 tooltip，私有/共享从灰底标签
  移进状态行（“离线 · darwin · 私有”）。离线机器的名称和计数降为次级墨色。容器宽度小于 30rem 时，
  三格一起换到名称下方。
- **已绑定账号是一张记录卡片。** 第一版把它改成单色名称加“绑定”菜单，被 owner 否掉（“做的并不好…
  就之前那种 boxed card”）。现在它像“我的机器”一样单独成组：每个服务一行，显示品牌标志、名称和状态
  （“已于 2025年1月15日 绑定”或“未绑定”），未绑定的行带“绑定”按钮，打开原有的确认对话框。移动端保留
  原来的一排 Logo。

未改动：标题处的 ghost 操作（“创建 Token”），以及用户名的 ghost 值按钮——其内边距仍让它比其他值
缩进约 10px。`@lody/ui` 没有导出按钮内边距 token，无法做对应的外扩。“我的机器”是否应放在账号页，是
留给 owner 的信息架构问题。
...
- 第四轮：设置浮层两种配色、中文，并强制打开桌面应用专属分组（偏好设置、外观、MCP、
  ...
  仍绘制卡片。
- 第五轮：`Settings/AccountSettings/DesktopPane`（在窗口作用域与 `settingsFlat` 内渲染账号页，
  含机器与 CLI Token），中文、两种配色、860px 与 480px 宽度的前后截图。
  `tests/account-machines-overview.test.tsx` 覆盖状态行；`tests/linked-accounts-list.test.tsx`
  覆盖每个服务的行状态，以及“绑定”按钮经确认到 `onConnect`。
