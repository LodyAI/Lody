...
机器胶囊改为页头里的下拉框，只在多于一台机器时显示，旁边是“添加 Provider”按钮。删除了
...
列表下方“可添加”列出这台机器尚未配置的内置运行时和预设，两列，名称加一句说明，点击即打开
已选中该 Provider 的添加对话框（`agent-config-dialog.tsx` 的 `listAddableProviders`；实验性
运行时、Registry Agent 和自定义命令仍只在对话框中）。列表短时它长，全部配置后它消失。以上
都不发起请求。
