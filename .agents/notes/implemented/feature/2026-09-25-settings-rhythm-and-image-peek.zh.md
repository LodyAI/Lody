...
针对真实桌面应用的第二轮评审，材质与间距保持不变：

- **键盘快捷键。** 15 个红色垃圾桶像 15 个警报。解绑按钮现在只在指针或键盘到达该行时出现，
  平时是中性墨色，只有悬停或聚焦在图标本身时才变成破坏性的红色。键帽文字提到 12px：
  `@lody/ui` 的 `Kbd` 新增 `size="medium"`（同一键帽里用 footnote 字号），命令面板和
  tooltip 仍用 11px。
- **一个有名字的添加。** 每个目录页的标题栏添加按钮都是图标加文字。项目页原来标题栏的纯图标
  “+”和每台机器上的“添加文件夹”打开同一个文件夹对话框，而对话框本身可以选机器，所以按机器
  的入口是重复的；现在只保留标题栏的“添加项目”（能添加 GitHub 仓库时是菜单，否则直接打开）。
- **关于。** 每行都把名字说两遍（官网 / 访问官网），“加入社区”是整个窗口唯一的主按钮。
  社区、官网、开源许可现在整行就是链接，行尾是安静的 ↗（离开 Lody）或 ›（打开对话框）
  （`CompactLinkRow`）；下载应用行用安静的链接列出两个渠道。行高保持控件高度，节奏不变。
- **Agents。** 英文 “Custom” 标签改为元信息行的第一项并本地化（“自定义 · 1 个对话 · 2 分钟前
  用过”）。“2m 前用过”把英文单位混进中文：`formatLocalizedRelativeTime` 按语言写出单位
  （`time.ago.*`，英文仍是 “2m ago”）。侧边栏的紧凑格式不变。
- **空目录。** MCP、Agent 角色、Prompt 快捷指令、分享、GitHub、项目以及没有 Provider 的
  机器共用 `SettingsEmptyList`：列表自己的记录卡片里一行安静的文字，不再有居中图标。
- **不可用的开关。** 开机自动启动只在 macOS 和 Windows 上支持（主进程的 `process.platform`
  判断）。不可用的行名称变淡（`CompactRow disabled`）并说明原因；“自动启动时隐藏窗口”说明
  它在开启开机自动启动后生效。
- **保留顶部内边距。** 去掉内容区顶部 20px 内边距后，页面标题与导航第一个分组标题、关闭按钮
  对齐（约 65px），但复审认为标题贴着面板顶边、没有页头留白。保留已认可的 20px 内边距；
  标题比导航分组标题低一行是有意为之。
