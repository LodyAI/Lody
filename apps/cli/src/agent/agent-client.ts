...
export type SteerPromptRun = {
  ...
  /** Provider delivery verdict; local Stop must not replace it. */
...
export class AgentClient implements acp.Client {
  ...
  steerPrompt(
    ...
    if (signal && !abortedBeforeSubmission) {
      // Stop releases only the local application waiter. The already-submitted
      // request remains the delivery verdict and stays in the session drain.
  ...
  private async requestSteeringExtension(
    ...
    // A submitted steer is raw ACP work even when its local application waiter
    // has been cancelled. Keep it in the same termination drain as prompts.
