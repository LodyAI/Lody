...
`auth login --claudeai` subscription flow. Codex runs `login --device-auth` only for
ChatGPT-backed configurations. A Codex configuration whose selected model provider sets
`requires_openai_auth = false` uses that provider's environment credential and never enters
the native ChatGPT login flow.
...
`codex login status` cannot account for provider-specific authentication. Because protocol
authentication spans launch preparation,
