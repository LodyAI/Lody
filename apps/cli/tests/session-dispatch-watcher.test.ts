...
describe('SessionDispatchWatcher', () => {
  ...
  it('dispatches an ACKed RPC turn when a rewrite barrier release requests a recheck', async () => {
    const startSession = vi.fn(async () => {});
    const sessionId = 'session-rpc-rewrite-barrier' as SessionId;
    const sessionMeta = {
      id: sessionId,
      machineId: 'machine-1',
      userId: 'user-1',
    ...
    await expect(
      watcher.offerRpcTurn({
        sessionId,
        userTurnId: 'rpc-turn-barrier',
        userId: 'user-1',
        timestamp: new Date().toISOString(),
        inputConfig: { prompt: 'send during reconciliation' },
      })
    ).resolves.toBe('accepted');
    await vi.waitFor(() => expect(getExecutionSnapshot).toHaveBeenCalled());
    expect(startSession).not.toHaveBeenCalled();

    barrierHeld = false;
    await watcher.enqueueSessionCheck(sessionId);

    expect(startSession).toHaveBeenCalledTimes(1);
    expect(startSession).toHaveBeenCalledWith(
      expect.objectContaining({ sessionId, userTurnId: 'rpc-turn-barrier' }),
      { dispatchSource: 'rpc' }
    );
  });
