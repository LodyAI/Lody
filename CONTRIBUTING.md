...
If an Agent prepares the pull request, it must tell the author-side user before submission that the Context handoff is public, an invalid body is closed after seven days, and a change over 200 lines without an Issue URL is closed immediately and must be resubmitted after issue discussion. The Agent must not claim that it gave this notice unless it actually did.
