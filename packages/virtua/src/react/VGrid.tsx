...
> {
   ...
   */
  children: (row: R, col: C, cell: Readonly<GridCellType>) => ReactNode;
  /**
...
interface GridCellProps {
  ...
  _children: (row: unknown, col: unknown, cell: Readonly<GridCellType>) => ReactNode;
  _row: unknown;
  _col: unknown;
  _rowIndex: number;
 ...
 */
export const VGrid = /*#__PURE__*/ forwardRef<VGridHandle, VGridProps<unknown, unknown>>(
  (
    {
      children,
      rows,
    ...
    const rowSizes = useMemo(() => rowLayout.$getSizes(rows, rowHeight), [rows, rowHeight]);
    const colSizes = useMemo(() => colLayout.$getSizes(cols, colWidth), [cols, colWidth]);
    ...
    const [stateVersion, rerender] = useReducer(getStateVersion, undefined, getStateVersion);
    ...
    const scrollToIndex = useLatestRef((opts: GridScrollToIndexOpts) =>
      gridScrollToIndex(driver, rowStore, colStore, rowLayout, colLayout, sectionRows, opts)
    );
    ...
    useIsomorphicLayoutEffect(() => {
      ...
      rowStore.$subscribe(UPDATE_SCROLL_EVENT, () => {
        scrolled = true;
        onVerticalScroll[refKey] && onVerticalScroll[refKey](rowStore.$getScrollOffset());
      });
      colStore.$subscribe(UPDATE_SCROLL_EVENT, () => {
        scrolled = true;
        onHorizontalScroll[refKey] && onHorizontalScroll[refKey](colStore.$getScrollOffset());
      });
...
) as <R = number, C = number>(props: VGridProps<R, C> & { ref?: Ref<VGridHandle> }) => ReactElement;
