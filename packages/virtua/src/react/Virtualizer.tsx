...
export interface VirtualizerProps<T = unknown> {
  ...
  shift?: boolean;
  /**
   * Lody: identify items by their React `key` instead of their index. Measured
   * sizes follow keys, a {@link CacheSnapshot} restores by key whatever the
   * item count, and when items are inserted, removed or moved the item at the
   * start of the viewport stays where it is. Needs element children with
   * stable keys; {@link VirtualizerProps.shift} is ignored.
   */
  keyed?: boolean;
  /**
 ...
 */
export const Virtualizer = /*#__PURE__*/ forwardRef<VirtualizerHandle, VirtualizerProps>(
  (
    {
      children,
      data,
    ...
    const [renderElement, count, getKeys] = useChildren(children, data);
    ...
    const [store, layout, driver, isHorizontal] = useStatic(() => {
      ...
      const initialKeys = keyed ? getKeys() : undefined;
      const _layout = createListLayout(count, itemSize, cache, initialKeys);
      const _store = createVirtualStore(_layout, ssrCount);
      if (initialKeys) _store.$setInitialKeys(initialKeys);
      return [_store, _layout, createContainerDriver(_store, _isHorizontal), _isHorizontal];
    });

    if (keyed) {
      // A no-op unless the keys changed (compared in the layout).
      store.$update(ACTION_ITEMS_KEYS_CHANGE, getKeys());
    } else if (count !== store.$getItemsLength()) {
...
) as <T>(props: VirtualizerProps<T> & { ref?: Ref<VirtualizerHandle> }) => ReactElement;
