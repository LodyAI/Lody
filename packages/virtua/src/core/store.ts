...
import type { ItemKey, ItemResize, ItemsRange } from './types.js';
...
const SCROLL_DOWN = 1;
const SCROLL_UP = 2;
type ScrollDirection = typeof SCROLL_IDLE | typeof SCROLL_DOWN | typeof SCROLL_UP;

const SCROLL_BY_NATIVE = 0;
const SCROLL_BY_MANUAL_SCROLL = 1;
const SCROLL_BY_SHIFT = 2;
type ScrollMode = typeof SCROLL_BY_NATIVE | typeof SCROLL_BY_MANUAL_SCROLL | typeof SCROLL_BY_SHIFT;
...
/** @internal Lody: the keyed item list changed. */
export const ACTION_ITEMS_KEYS_CHANGE = 10;
...
type Actions =
  ...
  | [type: typeof ACTION_ITEMS_LENGTH_CHANGE, arg: [length: number, isShift?: boolean | undefined]]
  ...
  | [type: typeof ACTION_ITEMS_KEYS_CHANGE, keys: readonly ItemKey[]];
...
/** @internal */
export type StateVersion = number & {}; /* hack for typescript to pretend as not falsy */
...
export type VirtualStore = {
  ...
  /** Lody: the keys a keyed layout was created with. */
  $setInitialKeys(keys: readonly ItemKey[]): void;
};
...
export const createVirtualStore = (
  ...
  // Lody: the keys the layout currently holds (keyed lists only).
  let prevKeys: readonly ItemKey[] | undefined;
  ...
  return {
    ...
    $update: (type, payload): void => {
      ...
      switch (type) {
          ...
          applyJump(
            updated.reduce((acc, [index, size]) => {
              ...
              if (
                ...
                shouldKeep =
                  _scrollDirection !== SCROLL_DOWN && _scrollMode === SCROLL_BY_NATIVE
        ...
        case ACTION_ITEMS_KEYS_CHANGE: {
          // Lody: keep the item at the start of the viewport where it is,
          // wherever it moved in the list. Virtua's index-keyed `shift` only
          // covers items added at the start; an insertion in the middle (a
          // placeholder becoming several items) otherwise moves the content
          // under the reader by the inserted size.
          const length = getLength();
          const visibleOffset = getVisibleOffset();
          let anchor: [key: ItemKey, offsetInItem: number][] = [];
          if (_isViewportMeasured && length && prevKeys) {
            // The first visible item, then the ones after it in case it is removed.
            const first = findIndex(max(0, visibleOffset));
            const last = min(length - 1, findIndex(visibleOffset + viewportSize));
            for (let i = first; i <= last; i++) {
              anchor.push([prevKeys[i]!, visibleOffset - getOffset(i)]);
            }
          }
          if (!setKeys!(payload)) break;
          prevKeys = payload;
          for (const [key, offsetInItem] of anchor) {
            const index = payload.indexOf(key);
            if (index < 0) continue;
            applyJump(getOffset(index) + offsetInItem - visibleOffset);
            break;
          }
          mutated = UPDATE_VIRTUAL_STATE;
          break;
