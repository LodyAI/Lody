import { ACTION_ITEM_RESIZE, ACTION_VIEWPORT_RESIZE, type VirtualStore } from './store.js';
import { createResizeObserver, createScrollObserver, type ScrollObserver } from './observer.js';
 ...
 * @internal
 */
export type DriverFactory = (store: VirtualStore, isHorizontal: boolean) => Driver;
...
export const createWindowDriver: DriverFactory = (store, isHorizontal) => {
  ...
  return {
    $observe(container) {
      ...
      const viewport = (viewportElement = document.scrollingElement! as HTMLElement);
      // Detect RTL document
      ...
      const calcOffsetToViewport = (
        ...
        const offsetSum =
          offset +
          (isRtl ? viewport.clientWidth - node[offsetKey] - node.offsetWidth : node[offsetKey]);
