...
type GridSizeFields<T> = {
  [K in Extract<keyof T, string> as T[K] extends GridTrackSize | null | undefined ? K : never]: 0;
};

export type GridSizeKey<T> = [T] extends [object] ? keyof GridSizeFields<T> : never;
...
export interface GridLayout extends Layout {
  ...
  $getSizes(axis: GridAxis<unknown>, size: GridTrackSize | string): GridAxisSizes | number | null;
  $setAxis(sizes: GridAxisSizes | number | null, scrollOffset: number): number | undefined;
}
...
export const createGridLayout = (
  ...
  return {
    ...
    $isSizeEqual: (index, size) => (sizes.length ? sizes[index] : UNCACHED) === size! + gap,
      ...
      typeof nextSize === 'number'
        ...
        : isUniform || isAuto || nextSize === 'auto'
          ...
          : (nextAxis as readonly unknown[]).map((item) =>
              ...
              item != NULL
                ? (item as Record<string, GridTrackSize | null | undefined>)[nextSize]
                : NULL
            ),
