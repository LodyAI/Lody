...
describe('estimateDefaultSize', () => {
  it('should not estimate the default size unless the whole axis is auto', () => {
    ...
    expect(createGridLayout(items('auto', 40), 's').$estimateDefaultSize).toBeUndefined();
  });
});
...
describe('setAxis', () => {
  describe('with keys', () => {
    ...
    it('should return the jump which keeps the visible position', () => {
      ...
      expect(layout.$setAxis(layout.$getSizes(items(150, 250, 300), 's'), 550)).toBe(50 + 50);
      // scrolled to the top: nothing above the anchor moves
      expect(layout.$setAxis(layout.$getSizes(items(100, 200, 300), 's'), 0)).toBe(0);
    });
    ...
    it('should update an item replaced in a copy of the axis', () => {
      ...
      expect(layout.$setAxis(layout.$getSizes(next.slice(), 's'), 0)).toBe(undefined);
    });
    ...
    it('should switch the key', () => {
      ...
      layout.$getTotalSize();
      expect(layout.$setAxis(layout.$getSizes([{ a: 100, b: 200 }], 'b'), 0)).toBe(0);
  ...
  describe('change detection', () => {
    it('should return undefined when nothing changed', () => {
      ...
      expect(layout.$setAxis(layout.$getSizes(items(100, 200), 's'), 0)).toBe(undefined);
    ...
    it('should ignore a size of another form', () => {
      ...
      expect(keyed.$setAxis(keyed.$getSizes(items(100, 200), 'auto'), 0)).toBe(undefined);
      expect(keyed.$setAxis(keyed.$getSizes(items(100, 200), 50), 0)).toBe(undefined);
...
describe('uniform size', () => {
  ...
  describe('findIndex', () => {
    ...
    it('should clamp the offset out of the items', () => {
      ...
      expect(layout.$findIndex(20 * CACHE_LENGTH + 1000)).toBe(CACHE_LENGTH - 1);
    });
  });
  ...
  describe('getItemOffset', () => {
    ...
    it('should get total - 1 item if index is at last', () => {
      expect(init().$getItemOffset(CACHE_LENGTH - 1)).toBe(20 * CACHE_LENGTH - 20);
    });
