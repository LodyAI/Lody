...
describe.each<{
  ...
  {
    name: 'list',
    init: (sizes) => createListLayout(sizes.length, undefined, [sizes.slice(), DEFAULT_SIZE]),
  ...
  },
  {
    ...
    snapshot: (layout) => {
      ...
      return [sizes, unmeasured < 0 ? DEFAULT_SIZE : layout.$getItemSize(unmeasured)];
    },
  },
])('$name', ({ init, snapshot }) => {
  ...
  describe('getRange', () => {
    ...
    describe.each([
      ...
      it('should get the index the range was computed from', () => {
        ...
        expect(layout.$getRange(start, start + 100)).toEqual([initialIndex, expect.any(Number)]);
      });
    });
  });
  ...
  describe('setLength', () => {
    ...
    it('should increase cache length', () => {
      const layout = init(range(10, () => UNCACHED));
      layout.$setLength(15);
      expect(snapshot(layout)).toEqual([range(15, () => UNCACHED), DEFAULT_SIZE]);
    ...
    it('should increase filled cache length', () => {
      ...
      const layout = initWithOffsets(sizes);
      layout.$setLength(15);
      expect(snapshot(layout)).toEqual([[...sizes, ...range(5, () => UNCACHED)], DEFAULT_SIZE]);
    ...
    it('should decrease cache length', () => {
      const layout = init(range(10, () => UNCACHED));
      layout.$setLength(5);
      expect(snapshot(layout)).toEqual([range(5, () => UNCACHED), DEFAULT_SIZE]);
  ...
  describe('getItemOffset', () => {
    ...
    it('should get total - 1 item if index is at last', () => {
      ...
      expect(layout.$getItemOffset(last)).toBe(sum(filledSizes) - filledSizes[last]!);
    });
  ...
  describe('estimateDefaultSize', () => {
    const initUnmeasured = (length: number) => initWithOffsets(range(length, () => UNCACHED));
    ...
    describe('start', () => {
      it('should update with 1 entry', () => {
        ...
        expect(layout.$getTotalSize()).toBe(
          initialTotalSize + (defaultSize! - initialDefaultSize!) * (100 - indexes.length)
        );
      });
