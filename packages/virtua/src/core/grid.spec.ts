...
import { createGridPlan, createGridSpanIndex, type GridPlan, type GridRowState } from './grid.js';

const NO_SPANS = createGridSpanIndex();

const rowStates = (plan: GridPlan): ReadonlyMap<number, GridRowState> =>
  new Map(plan.$groups.flatMap((g) => ('$rows' in g ? g.$rows : [g])).map((r) => [r.$row, r]));

const renderedCells = (states: ReadonlyMap<number, GridRowState>): string[] =>
  [...states].flatMap(([rowIndex, { $cells }]) => $cells.map((c) => rowIndex + '/' + c.$col));

const cell = (states: ReadonlyMap<number, GridRowState>, rowIndex: number, colIndex: number) =>
...
describe('rendered cells', () => {
  it('should render the cells in the ranges and the pinned ones', () => {
    ...
    const states = rowStates(createGridPlan(rowLayout, colLayout, [3, 4], [2, 2], NO_SPANS));
  ...
  it('should group only the rows pinned to each edge', () => {
    const rowLayout = createGridLayout(10, 40);
    rowLayout.$setPinned(1, 1);
    const plan = createGridPlan(rowLayout, createGridLayout(5, 100), [3, 4], [0, 4], NO_SPANS);
    expect(
      plan.$groups.map((g) => ('$rows' in g ? [g.$key, g.$rows.map((r) => r.$row)] : g.$row))
    ).toEqual([[-1, [0]], 3, 4, [-2, [9]]]);
    const rowLayout2 = createGridLayout(20, 40);
    rowLayout2.$setPinned(1, 1);
    // the groups keep their keys while the rows are added
    expect(
      createGridPlan(rowLayout2, createGridLayout(5, 100), [3, 4], [0, 4], NO_SPANS).$groups.map(
        (g) => ('$rows' in g ? g.$key : g.$row)
      )
    ).toEqual([-1, 3, 4, -2]);
    expect(
      createGridPlan(
  ...
  it('should render every cell once when the pinned counts exceed the grid', () => {
    ...
    const states = rowStates(createGridPlan(rowLayout, colLayout, [0, 1], [0, 1], NO_SPANS));
  ...
  it('should clamp the pinned end to the rest of the pinned start', () => {
    ...
    const states = rowStates(createGridPlan(rowLayout, colLayout, [0, 2], [0, 2], NO_SPANS));
    expect(renderedCells(states)).toEqual([
      '0/0',
      '0/1',
      '0/2',
  ...
  it('should render nothing without rows or columns', () => {
    expect(
      rowStates(
        createGridPlan(createGridLayout(0, 40), createGridLayout(5, 100), [0, -1], [0, 4], NO_SPANS)
      ).size
    ).toBe(0);
    expect(
      rowStates(
        createGridPlan(createGridLayout(5, 40), createGridLayout(0, 100), [0, 4], [0, -1], NO_SPANS)
      ).size
    ).toBe(0);
  });
  ...
  it('should skip the covered cells and give the spans to the origin', () => {
    const states = rowStates(
      createGridPlan(
        ...
        [0, 3],
        [0, 4],
        createGridSpanIndex([{ rowIndex: 1, colIndex: 1, rowSpan: 2, colSpan: 3 }])
      )
    );
  ...
  it('should not render a row whose cells are all covered', () => {
    const states = rowStates(
      createGridPlan(
        ...
        [0, 4],
        [0, 1],
        createGridSpanIndex([{ rowIndex: 1, colIndex: 0, rowSpan: 2, colSpan: 2 }])
      )
    );
...
describe('keepMounted', () => {
  it('should render only the kept cells outside the ranges', () => {
    ...
    expect(renderedCells(states)).toEqual(['0/0', '0/1', '0/50', '1/0', '1/1', '50/0', '60/60']);
  });
  ...
  it('should not render a kept cell covered by a span', () => {
    const states = rowStates(
      createGridPlan(
        ...
        [0, 2],
        [0, 4],
        createGridSpanIndex([{ rowIndex: 0, colIndex: 0, rowSpan: 2, colSpan: 2 }]),
        undefined,
  ...
  it('should render a kept cell pinned to the end once while the ranges are in the pinned end', () => {
    ...
    const plan = createGridPlan(rowLayout, colLayout, [7, 7], [7, 7], NO_SPANS, undefined, [
      { rowIndex: 6, colIndex: 6 },
    ]);
    expect(renderedCells(rowStates(plan))).toEqual(['6/6', '6/7', '7/6', '7/7']);
    expect(plan.$rowTemplate).toBe('[l0] minmax(240px,auto) [l6] 40px [l7] 40px [l8]');
    expect(plan.$colTemplate).toBe('[l0] minmax(600px,auto) [l6] 100px [l7] 100px [l8]');
  });
...
describe('headers', () => {
  it('should render a kept cell out of the ranges without its headers and the pinned cells', () => {
    ...
    const states = rowStates(
      createGridPlan(rowLayout, colLayout, [10, 11], [30, 31], NO_SPANS, undefined, [
...
describe('roles', () => {
  it('should make the pinned rows the column headers and the last pinned column the row header', () => {
    ...
    const colLayout = createGridLayout(3, 100);
    colLayout.$setPinned(2);
    const states = rowStates(createGridPlan(rowLayout, colLayout, [0, 4], [0, 2], NO_SPANS));
  ...
  it('should sort only the given header cell', () => {
    ...
    colLayout.$setPinned(1);
    const states = rowStates(
      createGridPlan(rowLayout, colLayout, [0, 4], [0, 2], NO_SPANS, undefined, undefined, {
        rowIndex: 0,
        colIndex: 1,
        order: 'ascending',
      })
    ...
    expect(
      cell(
        rowStates(
          createGridPlan(rowLayout2, colLayout2, [0, 4], [0, 2], NO_SPANS, undefined, undefined, {
            rowIndex: 2,
            colIndex: 0,
            order: 'descending',
          })
    ...
    expect(
      cell(
        rowStates(
          createGridPlan(rowLayout3, colLayout3, [0, 4], [0, 2], NO_SPANS, undefined, undefined, {
            rowIndex: 2,
            colIndex: 1,
            order: 'other',
          })
...
describe('measurement', () => {
  it('should measure an auto row by its first rendered cell without a row span', () => {
    const states = rowStates(
      createGridPlan(
        ...
        [0, 4],
        [0, 2],
        createGridSpanIndex([{ rowIndex: 1, colIndex: 0, rowSpan: 2 }])
      )
    );
  ...
  it('should measure an auto column by its first rendered cell without a column span', () => {
    const states = rowStates(
      createGridPlan(
        ...
        [0, 4],
        [0, 2],
        createGridSpanIndex([{ rowIndex: 0, colIndex: 1, colSpan: 2 }])
      )
    );
  ...
  it('should measure only the auto tracks of a key axis', () => {
    ...
    const states = rowStates(
      createGridPlan(rowLayout, createGridLayout(2, 100), [0, 2], [0, 1], NO_SPANS)
    );
    expect(cell(states, 0, 0).$measureRow).toBe(undefined);
...
describe('previous states', () => {
  ...
  it('should keep the states whose values are unchanged', () => {
    rows.$setPinned();
    cols.$setPinned();
    const prev = rowStates(createGridPlan(rows, cols, [0, 3], [0, 4], NO_SPANS));
    const next = rowStates(createGridPlan(rows, cols, [0, 3], [0, 4], NO_SPANS));
  ...
  it('should keep the rows rendered in both ranges', () => {
    ...
    cols.$setPinned();
    const prev = rowStates(createGridPlan(rows, cols, [0, 3], [0, 4], NO_SPANS));
    const next = rowStates(createGridPlan(rows, cols, [1, 4], [0, 4], NO_SPANS));
  ...
  it('should replace only the rows affected by a span', () => {
    ...
    const next = rowStates(
      createGridPlan(
        ...
        [0, 3],
        [0, 4],
        createGridSpanIndex([{ rowIndex: 2, colIndex: 0, colSpan: 2 }])
      )
    );
  ...
  it('should replace only the header cells whose sort changed', () => {
    ...
    const sorted = (colIndex: number) =>
      rowStates(
        createGridPlan(rows, cols, [0, 3], [0, 4], NO_SPANS, undefined, undefined, {
          rowIndex: 0,
          colIndex,
          order: 'ascending',
        })
...
describe('spans starting out of the ranges', () => {
  it('should span the headers rendered out of the ranges', () => {
    ...
    expect(renderedCells(states).filter((c) => c.endsWith('/0'))).toEqual(['0/0', '40/0']);
  });
  ...
  it('should not render a span over the end out of the ranges', () => {
    const states = rowStates(
      createGridPlan(
        ...
        [0, 5],
        [0, 4],
        createGridSpanIndex([{ rowIndex: 98, colIndex: 0, rowSpan: 5 }])
      )
    );
...
describe('sections', () => {
  ...
  it('should group the rows of the sections and stick their headers under the pinned rows', () => {
    rows.$setPinned(1);
    cols.$setPinned();
    const plan = createGridPlan(rows, cols, [6, 8], [0, 2], NO_SPANS, [5, 15]);
    const states = rowStates(plan);
    expect([...states.keys()]).toEqual([0, 5, 6, 7, 8]);
    expect(plan.$groups.map((g) => ('$rows' in g ? g.$key : g.$row))).toEqual([-1, -8]);
  ...
  it('should make the cell of a section header in the row header column the row header', () => {
    rows.$setPinned(1);
    cols.$setPinned(2);
    const states = rowStates(createGridPlan(rows, cols, [6, 8], [0, 2], NO_SPANS, [5]));
  ...
  it('should keep the rows before the first section out of the groups', () => {
    rows.$setPinned();
    cols.$setPinned();
    const plan = createGridPlan(rows, cols, [3, 6], [0, 2], NO_SPANS, [5]);
    expect(plan.$groups.map((g) => ('$rows' in g ? g.$key : g.$row))).toEqual([3, 4, -8]);
  ...
  it('should render the columns in the range in the section header sticking over them', () => {
    ...
    cols.$setPinned();
    const states = rowStates(
      createGridPlan(rows, cols, [7, 8], [5, 6], NO_SPANS, [5], [{ rowIndex: 20, colIndex: 8 }])
    );
    expect(renderedCells(states)).toEqual([
      '0/5',
      '0/6',
  ...
  it('should end the sections at the rows pinned to the end', () => {
    ...
    cols.$setPinned();
    const plan = createGridPlan(rows, cols, [26, 28], [0, 2], NO_SPANS, [25]);
    expect(groupOf(plan, -28)!.$style['gridRow']).toBe('l25/l29');
    expect(groupOf(plan, -28)!.$rows.map((r) => r.$row)).toEqual([25, 26, 27, 28]);
  });
  ...
  it('should not render the last section header for the cells pinned to the end', () => {
    ...
    const states = rowStates(
      createGridPlan(
        ...
        [1, 2],
        [0, 2],
        createGridSpanIndex([{ rowIndex: 29, colIndex: 0, colSpan: 2 }]),
        [15],
  ...
  it('should replace the header row when it starts sticking', () => {
    rows.$setPinned();
    cols.$setPinned();
    const plain = rowStates(createGridPlan(rows, cols, [4, 6], [0, 2], NO_SPANS));
    const sectioned = rowStates(createGridPlan(rows, cols, [4, 6], [0, 2], NO_SPANS, [5]));
  ...
  it('should take the section rows in the body in order', () => {
    rows.$setPinned(1, 1);
    cols.$setPinned();
    const plan = createGridPlan(rows, cols, [0, 29], [0, 2], NO_SPANS, [25, 5, 0, 29]);
  ...
  it('should make the consecutive section rows the sections of their own', () => {
    ...
    expect(plan.$groups.map((g) => ('$rows' in g ? g.$key : g.$row))).toEqual([-4]);
  ...
  it('should cut the tracks only at the ends of the rendered sections', () => {
    ...
    expect(plan.$groups.map((g) => ('$rows' in g ? g.$key : g.$row))).toEqual([-53, -503]);
