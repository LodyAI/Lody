...
const initCacheWithSizes = (sizes: readonly number[], defaultSize: number): TestCache => {
  return {
    _sizes: [...sizes],
...
const findIndexInCache = (cache: TestCache, offset: number, low?: number, high?: number) =>
...
describe(findIndex.name, () => {
  ...
  it('should get end if offset is at end + 1px', () => {
    ...
    expect(findIndexInCache(cache, sum(cache._sizes) + 1)).toBe(cache._length - 1);
  });
  ...
  it('should get end if offset is at end + 0.01px', () => {
    ...
    expect(findIndexInCache(cache, sum(cache._sizes) + 0.01)).toBe(cache._length - 1);
  });
  ...
  it('should get end if offset exceeds end', () => {
    ...
    expect(findIndexInCache(cache, Number.MAX_SAFE_INTEGER)).toBe(cache._length - 1);
  });
  ...
  it('should not get items with size 0', () => {
    ...
    expect(findIndexInCache(cache, sum(sizes) / 2)).toBe(sizes.findLastIndex((s) => s === 0) + 1);
  });
});
