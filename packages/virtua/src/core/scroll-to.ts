 ...
 * @internal
 */
export const scrollBy = (driver: Driver, store: VirtualStore, offset: number) => {
...
const scrollGridAxisToIndex = (
  ...
  const getInsetStart = (): number =>
    ...
    (sectionHeader < 0 || sectionHeader === index
      ? 0
      : layout.$getItemOffset(sectionHeader + 1) - layout.$getItemOffset(sectionHeader));
  ...
  if (align === 'nearest') {
    ...
    if (sectionHeader === index) {
      ...
      itemOffset = max(
        ...
        min(
          scrollOffset,
          store.$getItemOffset(lastIndex) + store.$getItemSize(lastIndex) - itemSize
        )
      );
    }
    if (itemOffset < scrollOffset) {
  ...
  driver.$scroll(isHorizontal, () => {
    const insetStart = getInsetStart();
    const rest =
      store.$getItemSize(index) - (store.$getViewportSize() - insetStart - getInsetEnd());
    return (
      store.$getItemOffset(index) -
      insetStart +
