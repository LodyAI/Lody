...
**后续设计决定（快照阶段已落地，不是 V4）：** 正常首次加入验证认证设备签署的权限快照，之后逐条验增量；全量历史保留供审计。入口 `verifySnapshot` / `openFromSnapshot` 已在 `70ffef7` 落地。核对必须绑定实际权限状态及链头，邀请者背书不等于独立核验全部历史。见[规范 §6.1](../../specs/e2ee-ledger.zh.md#61-签名快照引导已确认方向尚未实现)（标题保链接）。R2/C3 为后续验收，仍未验。
