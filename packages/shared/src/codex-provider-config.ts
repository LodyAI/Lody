...
function parseState(value: string | undefined): ProviderState | null {
  ...
  return {
    v: 1,
    previousModelProvider: previous.present
      ? { present: true, value: previous.value }
      : { present: false },
  };
}

export function isAllowedCodexEndpoint(value: string): boolean {
  let url: URL;
  try {
    url = new URL(value);
  } catch {
...
export function getLodyCodexCustomProvider(
  ...
  if (!state || config?.model_provider !== LODY_CODEX_MODEL_PROVIDER_ID) return null;
...
export function hasLodyCodexApiKeyAuthentication(
  env: Record<string, string | undefined> | undefined
): boolean {
  return Boolean(getLodyCodexCustomProvider(env) && env?.[LODY_CODEX_API_KEY_ENV]?.trim());
}
...
export function buildLodyCodexCustomProviderEnv(
  ...
  if (!isAllowedCodexEndpoint(baseUrl)) {
    throw new Error('Codex endpoints must use HTTPS, except for loopback HTTP');
  }
  ...
  next[LODY_CODEX_API_KEY_ENV] = apiKey;
  next[LODY_CODEX_PROVIDER_STATE_ENV] = JSON.stringify({
    v: 1,
