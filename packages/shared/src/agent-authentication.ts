...
import { hasLodyCodexApiKeyAuthentication } from './codex-provider-config';
 ...
 * reported by the agent itself. Codex's generated API-key provider is detected
 * from its complete config shape and referenced environment value.
 */
...
export const hasBuiltinEnvAuthentication = (
  ...
  return agentType === 'codex' && hasLodyCodexApiKeyAuthentication(env);
};
