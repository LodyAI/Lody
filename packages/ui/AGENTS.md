...
- Controls in the field family read validity and disabled from `Field.Root`
  ...
  are one fact; `src/field/invalid.ts` owns that reading, since StyleX cannot
  express an attribute selector.
- A trigger reads `field` and the list it opens reads `popup`: different rungs,
  and the list shares its vocabulary with a menu, not an input.
...
- Whatever holds a glyph gives it a box, because this package's glyphs and
  icons state 100% and StyleX has no descendant selector: a menu row's leading
  box, a badge's, an avatar fallback's, an icon-only `Button`'s. A caller's icon
...
- `Tooltip` is the one floating part not reading `popup`: the ladder inverts
  it, `label` under `shadow.medium`. Base UI makes it visual-only — no role, no
  `aria-describedby` — so every trigger states its `aria-label`.
...
- Alert, Toast, Progress, Skeleton and Spinner are one `feedback` family. A tone
  ...
  and `feedback/surface.ts` mixes the tint from its rung's background.
  `disabled` from Base UI state, not `:disabled`.
- `Table` and `Pagination` are one `table` family; Base UI ships neither.
  **A column is stated once**: `Table` takes `columns` and owns width, ordering
  (one column, on the root), selection (a `Checkbox`, never `aria-selected`),
  the empty row's span, the sticky head and the narrow stack. `table/parts.tsx`
  is the element layer. A table draws no surface: `border-collapse: separate`
  lets its line, head and row ring be box-shadows.
- Card, Badge and Separator answer "what is an edge?": a card's is its shadow
  ...
  pressable), a badge is on **no rung** so its tone is a film and its word that
  tone pulled halfway to ink, and a `Separator` _is_ the one allowed line: no
  token group, announced, no margin.
...
- Files calling `defineVars`, `createTheme` or `defineConsts` end in
  ...
  (`@lody/ui/tokens/colors.stylex`), never via a barrel.
