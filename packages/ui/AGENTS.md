...
- Checkbox, Radio and Switch render a real `<button>` through Base UI's
  `nativeButton`, so `:disabled` and `:focus-visible` reach them as they reach
  an `<input>`, and a `<label>` can point at them.
- Controls in the field family read validity and disabled from `Field.Root`
  ...
  own. The invalid ring follows `aria-invalid`, so it and what a screen reader
  announces are one fact; `src/field/invalid.ts` owns that reading in JS, because
  StyleX cannot express an attribute selector.
...
- `Select.Value` resolves its text from `Select.Root items`, never the rows; a
  caller whose row text differs from its value states the list there too, or the
  trigger names the raw value. `test/select.test.tsx` pins both.
- Every floating part's `Content` assembles Base UI's portal, positioner and
  ...
  container centred with `translate` is the containing block for `fixed` kin.
- `Menu` is the dropdown menu; `ContextMenu` and `Menubar` restate only the way
  ...
  list's declarations, `--anchor-width`; the surface and rows are shared through
...
- Whatever holds a glyph gives it a box, because this package's glyphs state
  ...
  badge's, a Select's chevron, a message's mark, and an icon-only `Button`,
  which boxes its children at `button.iconSize`. A caller's icon states its own
  size, not its library's default; a checkbox row's box holds its mark only.
- Every trigger here is Base UI's, unstyled: a surface opens a menu, a popover or
  a modal with what it already had there, through `render={<Button …/>}`, and
...
- Dialog, AlertDialog and Drawer are one family on the modal rung sharing
  ...
  names its panel to `PopupContainerProvider` in **state, not a ref**: React
  attaches a child's refs first, so a popup mounting in the same commit would
  read null and land on the body.
- There is no `Sheet`: a panel arriving from an edge is Base UI's `Drawer`, whose
  ...
  writing direction's edge, from which `Root` derives the swipe; `inset` is the
  second axis.
...
- Tabs, Accordion and Collapsible are one `disclosure` family sharing
  ...
  own indicator and states the size once for every tab in it; a revealed panel's
  padding rides on a child, because Base UI animates a height it measures with
...
- Alert, Toast, Progress, Skeleton and Spinner are one `feedback` family: what
  ...
  the part's, and `feedback/tone.ts` maps the four. The tint is mixed per rung in
  `feedback/surface.ts` — an Alert is a card, a Toast floats. `disabled` is Base
  UI state, not `:disabled`.
- Card, Badge and Separator are three answers to "what is an edge?". A card's is
  its shadow: the card rung, a Dialog's parts, and no nesting — a card in a card
  is one fill twice in the light palette. `interactive` only marks it pressable;
  the button stays the caller's. A badge is on **no rung**, so its tone is a film
  over whatever holds it and its words stay ink in all five. A `Separator` *is*
  the one allowed line: no token group, announced, and no margin of its own.
- A forced palette travels to a portalled popup. `ThemeRoot` publishes its mode
  and `Content` re-declares the palette on the positioner, because a popup mounts
  outside the subtree that declares it and would otherwise inherit the document's
  palette — a light panel on a dark page would open a dark list.
...
- Component tokens live beside the component as
  ...
  Input, Textarea, Checkbox, Radio, Switch and both triggers, `popup` the lists
  they open — rather than one group per component.
