...
- Collapsed local-project and GitHub-repository rows count fresh Session/child-Tab
  activity (including pinned Sessions) at the row end, before actions. Assign each Session
  to one state by permission > active > unread, then choose the project primary by
  permission > unread > active. Exclude its Session ids
