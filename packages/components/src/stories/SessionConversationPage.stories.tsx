...
const READING_ANSWER = [
  '## 阅读样式审查结论',
  '',
  '这一轮把对话页的正文调整到 **15px**、阅读栏保持 `768px`，并统一了暖色调。长段中文在这个宽度下每行大约 50 个字，',
  ...
  '| --- | --- | --- | --- | --- | --- | --- | --- |',
  '| Conversation prose | #D5D5D5 | #E4E1DD | 14.2:1 | 15px PingFang SC | 1.75 | 768px | Warm hue, HSL lightness 88% |',
  '| Interface text | #FFFFFF | #D2CDC5 | 11.6:1 | 14px | 1.5 | — | Menus, settings, buttons share one level |',
  '| Selected sidebar row | #F0EFED | #E4E1DD | 14.2:1 | 14px | 1.5 | — | Marked by its fill, not extra brightness |',
  '| Headings and bold | #EBEBEB | #EDE7DE | 15:1 | 17–19px | 1.4 | 768px | One step above prose |',
  '',
  '一张窄表格仍然贴合阅读栏：',
  '',
  '| 档位 | 字号 |',
  ...
  '```mermaid',
  'graph TD',
  '  A[Setting] --> B[15px prose]',
  '  A --> C[14px chrome]',
  '```',
  '',
  '```ts',
  'export const CONVERSATION_FONT_SIZES = [13, 14, 15, 16, 18] as const;',
  'export const uiFontSizeForConversation = (fontSize: number) => fontSize - 1;',
  '```',
  '',
