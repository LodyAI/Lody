...
export type AgentRunConfigMenuProps = {
...
/**
 * Reusable run-config picker: Machine → Agent (filtered) → Model / Reasoning
 ...
 * convention). Emits a configuration selection; the caller owns persistence and consent.
 */
export function AgentRunConfigMenu({
  value,
  onChange,
  disabled = false,
  trailing,
...
}: AgentRunConfigMenuProps) {
  const contentClassName = (...parts: (string | undefined | false | null)[]) =>
    cn(menuClassName, ...parts);
    ...
    explicitPermissionSelector || modeOptions.length === 0
      ? modeConfigSelector
        ? requireExplicitPermission
          ? (value?.configOptionValues?.[modeConfigSelector.configId] ?? null)
          : ((resolveConfigOptionValue(
              modeConfigSelector,
  ...
  const permissionLabel =
    permissionOptions.find((opt) => opt.value === permissionValue)?.label ??
    permissionValue ??
    null;
  ...
  const selectAgentConfig = useCallback(
    (config: AgentConfigMeta) => {
      ...
      const modeOptionsForDefault =
        ...
        [] as { value: string }[];
      const defaultMode = requireExplicitPermission
        ? undefined
    ...
    [commit, requireExplicitPermission]
  );
  ...
  useEffect(() => {
    ...
    if (requireExplicitPermission || explicitPermissionSelector) return;
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="start"
        className={contentClassName('min-w-64 max-w-80')}
        style={menuStyle}
      >
        {recents.length > 0 ? (
          <>
        ...
        <DropdownMenuSub>
          ...
          />
          <DropdownMenuSubContent className={contentClassName('min-w-52')} style={menuStyle}>
        ...
        <DropdownMenuSub>
          <ValueSubTrigger
            ...
            value={agentLabel}
          />
          <DropdownMenuSubContent className={contentClassName('min-w-56')} style={menuStyle}>
        ...
        {extraSelectSelectors.map((selector) => {
          ...
          return (
            <DropdownMenuSub key={selector.configId}>
              ...
              <DropdownMenuSubContent className={contentClassName('max-w-80')} style={menuStyle}>
        ...
        {modelPickerOptions.length > 0 ? (
          <DropdownMenuSub>
            ...
            <DropdownMenuSubContent
              className={contentClassName('max-w-80')}
              style={{
                ...menuStyle,
        ...
        {thinkingSelector ? (
          <DropdownMenuSub>
            <ValueSubTrigger
              ...
              value={thinkingLabel}
            />
            <DropdownMenuSubContent className={contentClassName()} style={menuStyle}>
        ...
        {permissionOptions.length > 0 ? (
          <DropdownMenuSub>
            ...
            />
            <DropdownMenuSubContent
              className={contentClassName('min-w-52 max-w-80')}
              style={menuStyle}
            >
