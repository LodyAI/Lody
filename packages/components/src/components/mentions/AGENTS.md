...
- `enableAtMentions` is the ONE list of what `@` reaches, gating trigger
  registration; every source with its own `enabled`
...
- URL references retain the pasted URL as native textarea text. Recognition runs
  ...
  a reference boundary opens the same actions as clicking it. The mention tree
  remains mounted for URL references even without any other mention source.
