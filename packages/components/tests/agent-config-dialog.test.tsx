...
describe('AgentConfigDialog', () => {
  ...
  it('stores Codex endpoint routing and its API key in the provider config env', async () => {
    const onSubmit = vi.fn(async (_payload: AgentConfigSubmitPayload) => {});
    await renderDialog(
      {
        kind: 'create',
        initialForm: { name: 'Codex API', cliType: 'builtin', agentType: 'codex' },
      },
      createMachine('Workstation', { providerSetup: PROVIDER_SETUP_PROTOCOL_VERSION }),
      onSubmit,
    ...
    expect(payload.env[LODY_CODEX_API_KEY_ENV]).toBe('sk-relay');
  ...
  it('edits Codex authentication per config without offering ChatGPT sign-in', async () => {
    const onSubmit = vi.fn(async (_payload: AgentConfigSubmitPayload) => {});
    const env = buildLodyCodexCustomProviderEnv(
      {},
      { baseUrl: 'https://relay.example.com/v1', apiKey: 'sk-existing' }
    );
    await renderDialog(
      {
        kind: 'edit',
