...
Honest-host authorization is a thin HTTP gateway in front of sqlite Riverrun. Riverrun stores ciphertext and performs CAS; it does not hold Org roles. The gateway re-reads the verified control ledger and applies membership, document-write, and key-distribution checks. Malicious-server tests continue to call `riverrunUrl` directly so a host 403 cannot stand in for client verification.
