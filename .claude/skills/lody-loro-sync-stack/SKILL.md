...
3. **Is presence still being produced locally?** Heartbeat writes are logged by `CliPresenceRuntime`. Continuing heartbeat logs prove local production only — the log line is written after the LOCAL store write and awaits no publish acknowledgement. Never read it as delivery.
